...
begin
  ...
  case Formula of
    ...
    EColorSpace.DeltaE:
      ...
      SimbaException('FindColorsOnBuffer: Formula invalid!');
  end;
  ...
  begin
    ...
    begin
      ...
      begin
        DebugLn('Outside of buffer: %d, (Lo: %d, Hi: %d)', [PtrUInt(Ptr), PtrUInt(BufferLo), PtrUInt(BufferHi)]); // is script process still monitored via IsProcessRunning?
        Halt;
      end;
      {$ENDIF}
...
var
  ...
  {$DEFINE MACRO_FINDCOLORS_BEGIN := Result := []; PointBuffer.Init(16*1024); }
  {$DEFINE MACRO_FINDCOLORS_MATCH := PointBuffer.Add(X + OffsetX, Y + OffsetY); }
  {$DEFINE MACRO_FINDCOLORS_END   := Result := PointBuffer.Trim(); }
  MACRO_FINDCOLORS

function CountColorsOnBuffer(Formula: EColorSpace; Color: TColor; Tolerance: Single; Multipliers: TChannelMultipliers;
                             Buffer: PColorBGRA; BufferWidth: Integer; SearchWidth, SearchHeight: Integer {$IFDEF SIMBA_BUFFERCHECKS}; BufferLo, BufferHi: PColorBGRA{$ENDIF}): Integer;

  {$DEFINE MACRO_FINDCOLORS_BEGIN := Result := 0; }
  {$DEFINE MACRO_FINDCOLORS_MATCH := Inc(Result); }
